...
def write_record(sheet_id, q, a, answer):
    ...
    questions = {question: (i, answer, int(all_count), int(correct_count), len([True for wrong_answer in wrong_answers if wrong_answer])) for i, (question, answer, all_count, correct_count, *wrong_answers) in enumerate(sheet_questions, 3)}
    if q in questions:
    ...
    if sheet_id in worksheets:
        worksheet = record_sh.worksheet(sheet_id)
        [title_load, common_question_load], *qs = worksheet.get_all_values()
        questions = {question: (i, int(count), int()) for i, (question, count) in enumerate(qs, 2)}
        if title_load != title:
            worksheet.update_cell(1, 1, title)
        if common_question_load != common_question:
            worksheet.update_cell(1, 2, common_question)
    else:
        worksheet = record_sh.add_worksheet(sheet_id, 0, 0)
        worksheet.append_row([title, common_question])
        questions = dict()
    
    if q in questions:
        count, i = questions[q]
        if a == answer:
            worksheet.update_cell(i, 2, count + 1)
    else:
        worksheet.append_row([q, a, 1])
    """